...
class _ChatWidgetState extends State<ChatWidget> {
  ...
  Future<void> fetchMessages() async {
    final response = await flightService.fetchMessages(
        widget.userType, widget.flightInfo, widget.userCode, 9);

    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      setState(() {
        messages =
            List<Map<String, dynamic>>.from(data['MENSAGEMS']); // Sem filtragem
      });
    } else {
  ...
  Future<void> sendMessage(String message) async {
    final response = await flightService.sendMessageAndPhoto(widget.userType,
        widget.userCode, widget.flightInfo, currentLoadedImage, message, 1);
